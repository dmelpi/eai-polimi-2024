...
struct _IAi_Application_Ispu_vtbl
{
  ...
  uint8_t (*ai_application_ispu_load_ucf)(IAi_Application_Ispu_t * _this, const char *filename, uint32_t ucf_content_size, const char *ucf_content);
};
...
{
  return _this->vptr->ai_application_ispu_load_ucf(_this, filename, ucf_content_size, ucf_content);
}
